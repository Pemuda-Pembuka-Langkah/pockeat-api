...
class FoodTextAnalysisService(BaseGeminiService):
    """Service for analyzing food descriptions using Gemini API."""
    
    def analyze(self, description: str) -> FoodAnalysisResult:
        """
        Analyze a food description and estimate nutritional content.
        
        Args:
            description: The food description to analyze.
            
        Returns:
            A FoodAnalysisResult with the analysis details.
            
        Raises:
            GeminiServiceException: If there's an error analyzing the food description.
        """
        try:
            prompt = f"""
            ...
            if isinstance(e, GeminiServiceException):
                raise
            raise GeminiServiceException(f"Failed to analyze food description '{description}': {str(e)}")
